...
class DroneBase(abc.ABC):
    r"""Base class for drones."""
    def __init__(
            self,
            ...
            obs_dim: int,
            time_step: float,
            aggregate_phy_steps: int,  # sub-steps used to calculate motor dynamics
        ...
        # Setup controller, e.g., PID, PWM, etc, 
        # details: https://www.bitcraze.io/documentation/repository/crazyflie-firmware/master/functional-areas/sensor-to-control/controllers/
        assert hasattr(control_modes, control_mode), \
            f'Control={control_mode} not found while initiating the drone.'
        control_cls = getattr(control_modes, control_mode)  # get class reference
...
class CrazyFlieAgent(DroneBase):
    def __init__(
            self,
            bc: bullet_client.BulletClient,
            control_mode: str,
        ...
        ##TODO: parameters changed by domain randomization
        ...
        #TODO: Note that A, B, K are manipulated by domain randomization
...
class CrazyFlieAgentWithDistb(CrazyFlieAgent):
    def __init__(
            self,
            bc: bullet_client.BulletClient,
            control_mode: str,
