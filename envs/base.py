"""  Base class for differemnt environements (tasks, like hover, tracking, etc.) which contains the drone and the physics
Basic codes: 
...
class EnvBase(gym.Env, abc.ABC):
    """Base class for all task environments."""
    metadata = {'render.modes': ['human', 'rgb_array']}

    def __init__(
            self,
            physics: str,  # e.g. PyBulletPhysics
            control_mode: str,  # use one of: PWM, Attitude, AttitudeRate
            drone_model: str,  # options: [cf21x_bullet, cf21x_sys_eq, cf21x_distb]
            init_xyz: np.ndarray,
            init_rpy: np.ndarray,
            ...
            motor_thrust_noise: float = 0.05,  # noise in % added to thrusts when using motor
            observation_frequency: int = 100,
            observation_history_size: int = 1,  # the number of history to be used for observation to the policy input
            observation_noise: float = 0.0,  # default: no noise added to obs
            sim_freq: int = 200,  # default: 200Hz, every 5ms update the simulation
            distb_type = None  # use this parameter to specify the type of distb
            # distb_level: float = 0.0,  # add this parameter for curriculum learning
            # adv_policy=None  # add this parameter for rarl
    ):
        """
        ...
        ------
        AssertionError
            If no class is found for given physics string or distb_type
        """
        ...
        # === Setup total update counter and disturbance type
        self.iteration = 0
        self.distb_type = distb_type
        assert self.distb_type in [None, 'constant', 'boltzmann', 'random', 'rarl', 'rarl-popularization'], \
            f'Invalid disturbance type: {self.distb_type}'
    ...
    def _setup_simulation(
        ...
        elif self.drone_model == 'cf21x_distb':
            self.drone = CrazyFlieAgentWithDistb(bc=self.bc, **self.agent_params)
        else:
            raise NotImplementedError

        # Setup forward dynamics - Instantiates a particular physics class.
        assert hasattr(forward_physics, physics), \
    ...
    def reset(self) -> np.ndarray:
        ...
        # Hanyang: avoid using domain randomization for now
